...
**A native Windows desktop app for configuring Cisco switches and routers — without memorizing a single command.**
...
**[⬇ Download the latest installer (v26.8.27)](https://github.com/AminBanaei/Cisco-Toolbox/releases/download/26.8.27/Cisco.Toolbox.26.8.27.Setup.exe)**

Run the installer and follow the setup wizard — no Python install required. See [all releases](https://github.com/AminBanaei/Cisco-Toolbox/releases) for older versions and release notes.
...
### Connectivity
- **SSH** and **Serial/COM port** (via `pyserial`), in one interface. SSH runs Windows' own `ssh` client (OpenSSH) as a subprocess and pipes its input/output straight into the app's terminal, so login (host-key confirmation, username, password) is handled interactively exactly like typing into a normal terminal window.
- Live terminal input/output — type directly, or click buttons.
...
- **Windows 10 or later.** The app is built and packaged for Windows.
...
- **An SSH client for SSH connections.** The app uses the standard `ssh` command — either a copy bundled with the app, or Windows' own OpenSSH Client. If neither is found, enable it once via **Settings → Apps → Optional Features → Add a feature → OpenSSH Client**. Serial connections don't need this at all.
...
**Optional — bundling `ssh.exe`:** drop a copy of `ssh.exe` in the project root (next to `main.py`) before building, and every build script will package it alongside the app automatically. That way SSH works out of the box on a target machine even if it doesn't have Windows' OpenSSH Client feature enabled. If you skip this, the app simply falls back to the `ssh` command on the system `PATH` at runtime.
...
CiscoToolbox/
  ...
  core/
    connection_manager.py    -> SSH (ssh.exe subprocess) / Serial (pyserial) session handling
