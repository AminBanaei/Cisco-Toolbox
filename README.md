...
Run the installer and follow the setup wizard — no Python or extra software required. See [all releases](https://github.com/AminBanaei/Cisco-Toolbox/releases) for older versions and release notes.
...
- **Windows 10 or later.** The installer sets everything up for you — nothing else to install first.
- **An SSH client for SSH connections.** The app uses the standard `ssh` command — either the copy already bundled with the installer, or Windows' own OpenSSH Client if that's missing. If neither is found, enable it once via **Settings → Apps → Optional Features → Add a feature → OpenSSH Client**. Serial connections don't need this at all.
